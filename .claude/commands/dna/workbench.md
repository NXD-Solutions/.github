...
- When scoping a CR's implementation notes, identify which files are derived (auto-regenerated) and exclude them explicitly. A derived file included in manual update scope creates work that conflicts with the generation mechanism. Example: `hierarchy.md` regenerates automatically — it must never appear in a "manually update" instruction.
- When migrating content between folders with different intent constraints, classify by current intent first. Wording debt (imperfect phrasing) is acceptable carry-forward; placement error is not. Classify, then create a follow-on CR for reword. Validated in `core/` migration: `communication.md` and `communicative-artefact.md` classified as universal before any reword discussion.
## Authoring lenses
...
8. **Review gate** — before creating the PR, verify:
   ...
   - Scope clause coverage — if the principle contains a scope clause ("before X", "when X", "while X"), does it inadvertently exclude domains the parent strand governs unconditionally? If yes, remove or widen the clause.
