...
| 2026-04-01 | artefact | .github — decision records, multiple sessions | Created one DR file per change instead of one DR file per node (H2) — multiple files now exist where one should accumulate all decisions for a node | User: "per earlier agreement that should be one DR file per principle/rule (h2)" — recurring violation | — | — |