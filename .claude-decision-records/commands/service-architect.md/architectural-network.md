...
**Decision:** Introduce a machine-readable service architecture as the source of truth
for BE code structure. The architecture is a graph of typed nodes (Process, Trigger, DataFetch
as initial vocabulary — living artefact). Stored as skill-managed markdown. The Service
Architect skill is the sole intended interface: it receives human requirements, translates
them to architecture changes, and derives a Confluence view for human readers. Humans may edit
files directly but are not required to.

**Related:** enables service-architect.md (the skill this decision warrants)
